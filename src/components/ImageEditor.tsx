...
export default function ImageEditor(props: ImageEditorPropsType) {
  ...
  // Reset settings and close modal
  const handleClose = () => {
    setSettings({
      type: "original",
      sensitive: false,
  ...
  return (
    <div className="flex items-center justify-center fixed w-screen h-screen inset-0 bg-black/75 z-10">
      <div className="bg-black rounded-xl p-10 flex flex-col gap-4">
        {/* Top */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-8">
            <svg
              onClick={handleClose}
              viewBox="0 0 24 24"
              className="w-6 h-6 rotate-180 cursor-pointer group"
            >
              <path
                fill="#e7e9ea"
                d="M7.414 13l5.043 5.04-1.414 1.42L3.586 12l7.457-7.46 1.414 1.42L7.414 11H21v2H7.414z"
                className="group-hover:fill-gray-400 effect"
              />
            </svg>
            <h2 className="font-bold">بریدن رسانه</h2>
          </div>
          <button
            onClick={onClose}
            className="py-2 px-4 rounded-full bg-white text-black font-bold cursor-pointer hover:bg-gray-300 effect"
          >
            ذخیره
          </button>
        </div>
